...
import CustomItemImageCarousel from './src/components/CustomItemImageCarousel.vue';

// Robuste Registrierung der Galerie-Komponente bei Ceres' Vue.
// Problem auf der Live-Seite: Ceres definiert window.Vue teils ERST NACHDEM dieses
// Skript geladen wurde. Ein einmaliges "if (window.Vue)" verpasst dann die
// Registrierung -> das <custom-item-image-carousel>-Tag bleibt leer.
// Lösung: sofort versuchen; sonst die window.Vue-Zuweisung abfangen (registriert
// exakt vor dem App-Mount) + kurzes Polling als Sicherheitsnetz.
function ensureCarouselRegistered() {
    if (typeof window.Vue === "undefined" || !window.Vue || !window.Vue.component) {
        return false;
    }
    if (!window.Vue.options.components['custom-item-image-carousel']) {
        window.Vue.component('custom-item-image-carousel', CustomItemImageCarousel);
    }
    return true;
}

if (!ensureCarouselRegistered()) {
    // window.Vue-Zuweisung durch Ceres abfangen -> im richtigen Moment registrieren
    try {
        var _ceresVue;
        Object.defineProperty(window, 'Vue', {
            configurable: true,
            enumerable: true,
            get: function () { return _ceresVue; },
            set: function (v) { _ceresVue = v; ensureCarouselRegistered(); }
        });
    } catch (e) {
        // window.Vue nicht (re)definierbar -> das Polling unten greift
    }

    // Sicherheitsnetz: kurzes Polling, bis Vue verfügbar ist
    var _regAttempts = 0;
    var _regTimer = setInterval(function () {
        if (ensureCarouselRegistered() || ++_regAttempts > 200) {
